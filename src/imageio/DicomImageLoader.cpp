...
// We mirror the DICOM display pipeline (PS3.3 C.11) as a sequence of *optional* stages so that callers can grab raw stored values,
// physical (modality-LUT-applied) values, or display values, whichever they want:
//
//   1. Stored pixel values   -- raw decoded integers (what GetBuffer hands us)
//   2. Modality LUT/Rescale  -- stored -> physical (e.g. Hounsfield units), via RescaleSlope/RescaleIntercept
//   3. VOI LUT/Windowing     -- physical -> display contrast, via WindowCenter/WindowWidth
//   4. Presentation/Photometric -- MONOCHROME1 inversion, PALETTE COLOR expansion, YBR->RGB, etc.
//
// GDCM is used purely as: container parser + transfer-syntax resolver + (de)compression of the pixel blob + metadata source. The math
// for stages 2-4 is kept here so that each can be toggled independently.
...
Task<vector<ImageData>> readDicomImage(std::istream& iStream, const ImageLoaderSettings& settings, int priority) {
    gdcm::ImageReader reader;
    reader.SetStream(iStream);
    if (!reader.Read()) {
        throw ImageLoadError{"File is not a readable DICOM image."};
    }
    ...
    // Series / instance identification for naming parts. SeriesNumber (0020,0011) + InstanceNumber (0020,0013) when present.
    const auto seriesNumber = dicomGetValue<0x0020, 0x0011, int>(ds);
    const auto instanceNumber = dicomGetValue<0x0020, 0x0013, int>(ds);

    if (seriesNumber) {
        partName = fmt::format("series.{}", *seriesNumber);
    }

    if (instanceNumber) {
        partName = Channel::joinIfNonempty(partName, fmt::format("inst.{}", *instanceNumber));
    }
    ...
    const auto readFrame = [&](size_t frameIdx) -> Task<ImageData> {
        ...
        resultData.partName = Channel::joinIfNonempty(partName, numFrames > 1 ? fmt::format("frame.{}", frameIdx) : "");
        ...
        // them into a sensible [0,1] display range using the stored bit depth so the image is at least viewable. This is the "give me raw
        // but show me something" fallback; callers wanting true raw values can disable it.
    ...
    vector<ImageData> resultData;
    for (size_t i = 0; i < numFrames; ++i) {
        resultData.emplace_back(co_await readFrame(i));
    }

    co_return resultData;
}

Task<vector<ImageData>> readDicomDir(const gdcm::File& dirFile, const fs::path& path, const ImageLoaderSettings& settings, int priority) {
    ...
    const auto loadFile = [&](size_t fi) -> Task<vector<ImageData>> {
        ...
        try {
            co_return co_await readDicomImage(refStream, settings, priority);
    ...
    vector<Task<vector<ImageData>>> loadTasks;
    ...
    vector<ImageData> dirResult = co_await awaitAll(loadTasks) | views::join | toVector;
    if (dirResult.empty()) {
...
Task<vector<ImageData>>
    ...
    if (!iStream || !hasMagic) {
        ...
        if (lowerExt == ".dcm" || lowerExt == ".dicom") {
            tlog::warning("File has DICOM extension but missing magic; attempting lenient parse.");
        } else {
    ...
    try {
        ...
        reader.SetStream(iStream);
        if (!reader.Read()) {
            throw FormatNotSupported{"File is not a readable DICOM file."};
        }
        ...
        if (ms == gdcm::MediaStorage::MediaStorageDirectoryStorage) {
            ...
            co_return co_await readDicomDir(dirFile, path, settings, priority);
        }

        // Not a DICOMDIR; try loading as a normal DICOM image
        iStream.clear();
        iStream.seekg(0);

        co_return co_await readDicomImage(iStream, settings, priority);
