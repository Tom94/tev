...
Xmp::Xmp(string_view xmpData) {
    ...
    try {
        ...
        while (iter.Next(&schema, &path, &value)) {
            // tlog::debug() << format("{} | {} | {}", schema, path, value);
            ...
            if (!node->value.empty()) {
                tlog::warning()
                    << format("XMP property '{}' already has a value '{}', overwriting with new value '{}'.", path, node->value, value);
            }
        ...
        if (XMP_Int32 orientation; meta.GetProperty_Int(kXMP_NS_TIFF, "Orientation", &orientation, nullptr)) {
            switch (orientation) {
                ...
                default:
                    tlog::warning() << format("Invalid XMP orientation value: {}", orientation);
                    mOrientation = EOrientation::None;
                    break;
            }

            tlog::debug() << format("Found XMP orientation: {}", toString(mOrientation));
        }
        ...
        // IFD, but the XMP metadata on the child image indicates whether there's a gain map.
        {
            ...
            if (string prefix, version;
                ...
                tlog::debug() << format("Found Apple HDR gain map metadata: prefix={} version={}", prefix, version);

                if (string headroom; meta.GetProperty(hdrGainMapNs, "HDRGainMapHeadroom", &headroom, nullptr)) {
                    tlog::debug() << format("- HDRGainMapHeadroom: {}", headroom);
                }
            }
            ...
            if (string prefix; meta.GetNamespacePrefix(pixelDataNs, &prefix) &&
                ...
                tlog::debug() << format("Apple aux image: prefix={} type={}", prefix, mAppleAuxImgType);
            }
        }
        ...
        try {
            ...
            if (string prefix, version; meta.GetNamespacePrefix(ns, &prefix) && meta.GetProperty(ns, "Version", &version, nullptr)) {
                tlog::debug() << format("Found XMP gainmap metadata: prefix={} version={}", prefix, version);
        ...
        } catch (invalid_argument& e) { tlog::warning() << format("Failed to convert XMP gainmap metadata: {}", e.what()); }
